...
const ShowList = () => {
    ...
    const fetchShows = async () => {
      try {
        if (selectedDate && selectedTime && token) {
          const response = await axios.get(`http://localhost:8000/shows-at-time/${selectedTime}`, {
            headers: {
  ...
  return (
    <div>
      ...
      <div style={{ margin: '0 auto', maxWidth: '100%' }}>
        <div>
          ...
          {selectedDate && selectedTime && !error && (
            <>
              <h2 style={{ textAlign: 'center' }}>Shows on {selectedDate} at {selectedTime}</h2>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
                {shows.map(show => (
                  <div key={show.id} style={{ border: '1px solid #ccc', padding: '10px' }}>
                    <Link to={`/hh/${show.id}`}>
                      <h4 style={{ marginBottom: '5px', textDecoration: 'none' }}>{show.title}</h4>
                    </Link>
                    <p style={{ marginBottom: '5px' }}>Release date: {show.date}</p>
      ...
      <footer style={{ backgroundColor: 'rgb(211, 211, 211)', padding: '30px', marginTop:'200px' }}>
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
    <div>
      <h3>About Our Movie Site</h3>
      <p>
        Welcome to our movie site! We strive to provide you with the best selection of movies and TV shows, 
        along with a seamless booking experience. Enjoy exploring our collection and book your tickets today!
      </p>
    </div>
    <div>
      <h3>Contact Us</h3>
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
      ...
      <div style={{ display: 'flex', gap: '10px' }}>
        <a href="https://www.facebook.com/moviesite" target="_blank" rel="noopener noreferrer">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" style={{ fill: 'grey' }}>
        ...
        <a href="https://www.twitter.com/moviesite" target="_blank" rel="noopener noreferrer">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="24" height="24" style={{ fill: 'grey' }}>
            <path d="M0 0h24v24H0z" fill="none" />
